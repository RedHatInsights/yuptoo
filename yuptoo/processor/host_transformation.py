...
def remove_invalid_bios_uuid(
        host, transformed_obj=copy.deepcopy(TRANSFORMED_DICT)):
...
def transform_os_release(
    ...
    os_details = match_regex_and_find_os_details(os_release)
...
def transform_network_interfaces(
    ...
    for nic in filtered_nics:
        increment_counts, nic = transform_mtu(
            nic, increment_counts)
        increment_counts, nic = transform_ipv6(
            nic, increment_counts)
